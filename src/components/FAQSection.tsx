...
export const FAQSection: React.FC<FAQSectionProps> = ({ onOpenLeadModal }) => {
  ...
  return (
    <section id="faq" className="py-20 lg:py-24 bg-white relative border-t border-slate-200">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 relative">
          ...
          <input
            ...
            className="w-full ps-11 pe-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-xs"
          />
        </div>
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-5 text-start flex items-center justify-between gap-4 focus:outline-none"
        ...
        <div className="p-6 rounded-3xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-start">
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-slate-900">لديك استفسار خاص بحالتك ومعدلك؟</h4>
            <p className="text-xs text-slate-600 font-medium">مستشار جودة جاهز للإجابة الفورية عبر واتساب مجاناً.</p>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-2.5 shrink-0">
            <button
              onClick={() => onOpenLeadModal('faq_custom_question')}
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold bg-slate-900 text-white hover:bg-slate-800 transition-colors"
            >
            ...
            <a
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold bg-emerald-600 text-white hover:bg-emerald-500 transition-colors shadow-xs"
            >
