...
export const StudentJourneyVisual: React.FC<StudentJourneyVisualProps> = ({ onOpenLeadModal }) => {
  ...
  return (
    <section id="journey" className="py-20 lg:py-24 bg-white relative border-t border-slate-200">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5 mb-8">
          {stages.map((st, idx) => (
            <button
              ...
              className={`p-3 rounded-2xl border text-center transition-all focus:outline-none ${
                activeStage === idx
                  ? 'bg-emerald-600 text-white border-emerald-600 shadow-md shadow-emerald-600/20 font-bold scale-105'
                  : 'bg-slate-50 text-slate-700 border-slate-200 hover:border-slate-300 hover:bg-slate-100'
              }`}
            >
              <div className="text-xs font-sans opacity-90">{st.num}</div>
              <div className="text-[11px] font-bold truncate mt-1">{st.title}</div>
            </button>
          ))}
        </div>

        {/* Active Stage Spotlight Card */}
        <div className="rounded-3xl bg-slate-50 border border-slate-200 p-6 sm:p-8 shadow-md">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <button
              ...
              className="shrink-0 flex items-center gap-2 px-6 py-3.5 rounded-2xl text-xs font-bold bg-emerald-600 text-white hover:bg-emerald-500 transition-all shadow-md"
            >
